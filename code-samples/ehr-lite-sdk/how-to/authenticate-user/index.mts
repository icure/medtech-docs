...
import { host, output, password } from '../../../utils/index.mjs'
import { getLastEmail } from '../../../utils/msgGtw.mjs'
import { TestVars } from '@icure/test-setup/types'
import { username } from '../../../medtech-sdk/quick-start/index.mjs'
...
// TODO Remove master hcp api when we get rid of the hcp id parameter
...
//tech-doc: Instantiate anonymous API
...
output({
  anonymousApi:
    'An initialized AnonymousEHRLiteApi instance. ' +
...
const authProcess = await anonymousApi.authenticationApi.startAuthentication(
  ...
  undefined, // Phone number of the user who wants to register. Not necessary if the email is provided.
  'John',
  'Doe',
  masterHcpId, // skip TODO we have to remove the hcp id from the api
)
//tech-doc: STOP HERE
output({ authProcess })
...
const currentUser = await authenticatedApi.userApi.getLogged()
currentUser.passwordHash = forceUuid()
await authenticatedApi.userApi.createOrModify(currentUser)
