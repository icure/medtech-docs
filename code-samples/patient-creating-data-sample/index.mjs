...
;
global.localStorage = new LocalStorage(tmpdir(), 5 * 1024 ** 3);
global.Storage = '';
...
const api = await medTechApi()
    ...
    .build();
await api.cryptoApi.loadKeyPairsAsTextInBrowserLocalStorage(hcpId, hex2ua(privKey));
const patient = await api.patientApi.getPatient(patientId);
const healthcareElement = await api.healthcareElementApi.createOrModifyHealthcareElement(new HealthcareElement({
