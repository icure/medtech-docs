...
A Content is an encryptable entity that contains the value of the medical data to store in the cloud. It is nested 
inside [Service](/nsdk/explanations/data-model/service), and it will always be encrypted before being saved in the cloud.
...
If you want to store large files / big binary content, you can store it in a [Document](nsdk/explanations/data-model/document)
and put the document id in the [documentId](/nsdk/explanations/data-model/content#documentid) field of the Content.

More information about how to store large files are available in [this how to](/nsdk/how-to/store-big-files).
:::
...
[Service](/nsdk/explanations/data-model/service)s. 

### documentId
The id of a [Document](nsdk/explanations/data-model/document) entity. This can be used to store large files as medical
content without having to include the file directly in the entity.

### fuzzyDateValue
A date to store in the Content as a *fuzzy date*, i.e. a numeric value in the format `YYYYMMDDhhmmss`.
