...
async function shareHealthElementForStatistics(
	...
	const shareResult = await sdk.healthElement.shareWith(
		// You already know the data owner id
		STATISTICS_DATA_OWNER_ID,
		healthElement,
		{
	...
	if (shareResult instanceof SimpleShareResult.Success) {
		return shareResult.updatedEntity
...
from cardinal_sdk.model import DecryptedHealthElement, HealthElementShareOptions, RequestedPermission, \
	ShareMetadataBehaviour, SecretIdShareOptionsUseExactly, SimpleShareResultDecryptedHealthElementSuccess
...
def share_health_element_for_statistics(
	...
	share_result = sdk.health_element.share_with_blocking(
		# You already know the data owner id
		__STATISTICS_DATA_OWNER_ID,
		health_element,
		HealthElementShareOptions(
	...
	if isinstance(share_result, SimpleShareResultDecryptedHealthElementSuccess):
		return share_result.updated_entity
	else:
