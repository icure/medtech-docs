...
that subscribes to the creation of new `Service` objects with the tag `INTERNAL_INFERENCE_STATUS` having the value
...
The `subscribeToServiceCreateOrUpdateEvents` method allows you to subscribe to the creation or update events of a
`Service`. In this example, we filter the events to receive only those with the tag type `INTERNAL_INFERENCE_STATUS`
...
sdk.contact.subscribeToServiceCreateOrUpdateEvents(
    ...
    println("Service created or updated: $it")
}
```
...
The `subscribe_to_service_create_or_update_events_blocking` method allows you to subscribe to the creation or update events of a
`Service`. In this example, we filter the events to receive only those with the tag type `INTERNAL_INFERENCE_STATUS`
and the tag code `TO_BE_STARTED`.
