...
export const preTestProvides: Record<string, string[]> = {
	'define-user-roles block 1 (line 52)': [],
	'define-user-roles block 2 (line 85)': ['allRoles', 'userId'],
	'define-user-roles block 3 (line 132)': ['userId'],
}

// ── preTest ──────────────────────────────────────────────────────────

export const preTest: Record<string, (sdk: CardinalSdk) => Promise<Record<string, any>>> = {
	'define-user-roles block 1 (line 52)': async () => {
		return {}
	},

	// Block 2 references `allRoles` from block 1.
	'define-user-roles block 2 (line 85)': async (sdk) => {
		if (!allRoles) {
	...
	'define-user-roles block 3 (line 132)': async (sdk) => {
		if (!userId) {
			const currentUser = await sdk.user.getCurrentUser()
			userId = currentUser.id
		}
...
export const postTest: Record<string, (...args: any[]) => void | Promise<void>> = {
	...
	'define-user-roles block 1 (line 52)': async (_sdk: CardinalSdk, allRolesLocal: any[]) => {
	...
	'define-user-roles block 2 (line 85)': async (_sdk: CardinalSdk, userIdLocal: string, user: User) => {
		if (userIdLocal && user) {
	...
	'define-user-roles block 3 (line 132)': async (sdk: CardinalSdk) => {
