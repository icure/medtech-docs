...
The iCure SDKs encrypt and decrypt sensitive data on the client-side (end-to-end), this way the sensitive data
travelling over the internet and stored in our servers can't be read by any third party.
...
Asymmetric cryptography algorithms instead use pairs of keys, and each pair is composed of a `private key`, which must be
