...
export default function Home(): JSX.Element {
    ...
    return (
        useMemo(() => <Layout title={`${siteConfig.title}`}
                              description="Description will go into a meta tag in <head />">
                <main className={styles.homePage}>
                    <section className={`${styles.homePage__intro} ${styles.container}`}>
                        <h1>{`${siteConfig.title}`}</h1>
                        <p>
                            iCure is a <strong>trustable</strong> service ensuring the <strong>privacy</strong> of your
                            patient <strong>medical data</strong> thanks to{' '}
                            <strong>end-to-end encryption</strong>, that empowers your users to decide who can access their
                            data.
                        </p>
                    </section>
                    ...
                    {AppsList(code).map((element, index) => {
                        ...
                        return (
                            <section key={`apps-section-${index}`}
                                className={`${styles.homePage__appsCardContainer} ${index % 2 !== 0 && styles.homePage__appsCardContainerReverse}`}>
